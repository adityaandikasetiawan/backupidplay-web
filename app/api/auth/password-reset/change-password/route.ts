...
export async function PATCH(req: NextRequest) {
  ...
  const urlPath = `${baseUrl}user/change-password`;
  ...
  try {
    ...
    if (!resp.ok) {
      logApiError({
        ...
        resp,
        data,
        message: 'API call to change-password endpoint failed.',
      });
      return NextResponse.json(
    ...
    logCaughtError({
      req,
      error,
      message: 'An unexpected error occurred in the change-password route.',
    });
